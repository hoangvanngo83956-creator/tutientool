...
export async function factCheckScript(input: { script: unknown; evidence: string }): Promise<ScriptFactCheckResult> {
  ...
  return validate(parseAIJson(content, "Script fact checker returned invalid JSON format."));
}
...
function validate(value: unknown): ScriptFactCheckResult {
  ...
  return {
    ...
    checks: checks.slice(0, 30).map((item: any) => ({
      claim: String(item.claim || ""),
      status: ["supported", "weakly_supported", "unsupported"].includes(item.status) ? item.status : "weakly_supported",
      reason: String(item.reason || "Can kiem tra lai."),
      evidence_reference: item.evidence_reference ? String(item.evidence_reference) : null
    })),
...
function normalizeVisualPromptCheck(value: any) {
  ...
  return {
    ...
    checks: checks.slice(0, 20).map((item: any) => ({
      scene_index: Number(item.scene_index) || 0,
      status: ["passed", "needs_review", "failed"].includes(item.status) ? item.status : "needs_review",
      reason: String(item.reason || "Can kiem tra visual prompt."),
